...
my $winner_id = top_key_by_value(\%asleep);

my $top_min = top_key_by_value($hist{$winner_id});
...
while (my($id, $h) = each %hist) {
  my $min = top_key_by_value($h);
  $top_min_per_guard{$id} = $min;
  $most_min_per_guard{$id} = $h->{$min};
}

my $winner2_id = top_key_by_value(\%most_min_per_guard);
